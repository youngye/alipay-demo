...
func init(){
	s_nCount	=0
	s_ch		=make(chan int, 1)
	logfile, _ := os.OpenFile("alipay.log", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	if logfile != nil {
		//defer logfile.Close()
...
//按照支付宝规则生成MD5加密sign
func alipayMd5Sign(sStr string) string {
	sStr, err	:=url.QueryUnescape(sStr)
	if(err != nil){
		return ""
	}
	sStr	+=s_sAlipayKey
	m		:=md5.New()
	m.Write([]byte(sStr))
	sStr	=hex.EncodeToString(m.Sum(nil))
	return sStr
}
...
//发起WEB支付请求
func alipayRequest_web(orderId string, sSubject string, sFee string) string {
	const STR_SERVICE	="create_direct_pay_by_user"	
	urls	:=&url.Values{}
	urls.Set("service", STR_SERVICE)
	urls.Set("partner", s_sAlipayPartner)
	urls.Set("_input_charset", s_sCharset)
	urls.Set("payment_type", "1")
	urls.Set("notify_url", s_sWebNotifyUrl)
	urls.Set("out_trade_no", orderId)
	urls.Set("return_url", s_sWebCallbackUrl)
	urls.Set("seller_email", s_sSellerEmail)
	urls.Set("subject", sSubject)
	urls.Set("total_fee", sFee)
	sSign	:=alipayMd5Sign(urls.Encode())
	return `
		<html>
		<head>
		...
		<body>
		<form id="alipaysubmit" name="alipaysubmit" action="` +s_sAlipayGatewayWeb +"_input_charset=" +s_sCharset +`" method="get" style='display:none;'>
			<input type="hidden" name="_input_charset" value="` +s_sCharset + `">
			<input type="hidden" name="notify_url" value="` +s_sWebNotifyUrl + `">
			<input type="hidden" name="out_trade_no" value="` +orderId + `">
			<input type="hidden" name="partner" value="` +s_sAlipayPartner + `">
			<input type="hidden" name="payment_type" value="1">
			<input type="hidden" name="return_url" value="` +s_sWebCallbackUrl + `">
			<input type="hidden" name="seller_email" value="` +s_sSellerEmail + `">
			<input type="hidden" name="service" value="` +STR_SERVICE + `">
			<input type="hidden" name="subject" value="` +sSubject + `">
			<input type="hidden" name="total_fee" value="` +sFee + `">
			<input type="hidden" name="sign" value="` +sSign + `">
			<input type="hidden" name="sign_type" value="` +s_sSigntype + `">
...
func AlipayHandler_Web(w http.ResponseWriter, r *http.Request){
	...
	html		:=alipayRequest_web(getUniqueID(), sSubject, sFee)
	w.Write([]byte(html))
}
